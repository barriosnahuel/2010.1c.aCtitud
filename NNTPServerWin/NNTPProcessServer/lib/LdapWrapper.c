...
VOID buildLDAPMod(PLDAP_ENTRY object) {
	PLDAP_ATTRIBUTE currentAttrib;
	INT	index;

	object->ldapMod = __LDAP_ALLOC(LDAPMod **, LDAPMod *, object->actualSize + 1);
	currentAttrib = object->firstAttribute;
	index = 0;
...
VOID addEntry(PLDAP_SESSION session, PLDAP_ENTRY object) {
	...
	returnValue = ldap_add_ext_s(session->context->ldap, object->dn, object->ldapMod, NULL, NULL);
...
VOID editEntry(PLDAP_SESSION session, PLDAP_ENTRY entry) {
	...
	returnValue = ldap_modify_ext_s(session->context->ldap, entry->dn, entry->ldapMod, NULL, NULL);
...
PLDAP_ATTRIBUTE createAttributeFromDoublePointerList(PCHAR attributeName, INT valueSize, PPCHAR wordsList) {
	PLDAP_ATTRIBUTE attribute 		= NULL;
	LDAPMod 		*modObj			= NULL;
	PPCHAR			attributes 		= NULL;
	int	 		index;
	...
	for(index = 0; index < valueSize; index++) {
		...
		attributes[index] = wordsList[index];
